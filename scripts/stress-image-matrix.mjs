...
function parseArgs(argv) {
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (arg === '--base-url' && next) {
      ...
      args.mode = next === 'image-plus-oracle-text' ? next : 'image-only';
      index += 1;
    }
  }
  return args;
...
function evaluateOracle(test, room) {
  ...
  assertExpectedCount(test, 'calculation rule', room?.calculationRules || [], oracle.expectedCalculationRules?.count, oracle.expectedCalculationRules?.countAtLeast);
...
async function uploadEvidence(args, roomId, test, imageBuffer) {
...
async function runOne(args, test) {
  ...
  const parsed = await uploadEvidence(args, room.id, test, image.buffer);
  evaluateOracle(test, parsed);
  return {
    id: test.id,
    ok: true,
    roomId: room.id,
