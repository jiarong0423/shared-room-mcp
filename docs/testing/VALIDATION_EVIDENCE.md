...
|---|---:|---|---|
| Current Zeabur post-deploy health | PENDING RECHECK | deploy this commit, then recheck live `/healthz` | must return `providerOrder=["local_vision","gemini","openai"]`, `localVisionConfigured=false`, `allowRemoteVisionFallback=false`, `/data/rooms.json`, and no provider key flags |
...
| Current local guided OCR+LLM proposal apply smoke | PASS | `/private/tmp/webmcp-commercial-ui-codex-demo-v3/codex-ocr-llm-demo-2026-09-03T20-36-18-486Z.json` | Tesseract OCR produced 7 draft rows from the generated menu image; guided Codex visual review prepared 18 structured rows; the test-only accept flag applied 18 rows and kept customer publishing closed |
| Current local guided OCR+LLM pending demo room | PASS | `/private/tmp/webmcp-codex-ocr-llm-local-pending-final-2/codex-ocr-llm-demo-2026-09-03T18-44-40-257Z.json` | same generated evidence and OCR path, but left the 18-row Codex visual-review draft pending for the merchant to approve |
...
| Current HITL Customer Publishing after visual-review proof gate | 8/8 passed | `/private/tmp/webmcp-commercial-ui-customer-publishing-v2/customer-publishing-stress-2026-09-03T20-35-20-574Z.md` | visual-review-backed drafts remain behind merchant review, then customer publishing, customer confirmation, merchant finalization, language-locked HTML export, and PDF export pass |
| Current browser/WebMCP product-room smoke | PASS | Codex in-app browser against `http://127.0.0.1:3224` | English sample room showed no old engineering UI strings or Chinese system text; `inspect_room` returned `productRoom`, `commercialFlow`, merchant/customer roles, scoped-room privacy, and proposal product status |
...
| Local OCR canary | 3/3 local S01 variants passed | local isolated runner output outside the repository | operator-machine Tesseract OCR plus text-block parsing produced customer-safe candidates, prevented age-number leakage, and kept group-threshold text as advisory merchant context rather than a Zeabur OCR claim |
...
- OCR-only output is draft evidence. A cloud `semantic_repair_draft` requires local OCR plus local/visual LLM structured items before merchant review can clear the OCR-only blocker.
...
- The recording runbook now fixes the human/agent handoff: the presenter/operator handles page navigation and file selection; WebMCP and the assistant handle room-state reading, local-review draft preparation, and publishing-gate checks; the human performs the same-card two-click review, customer confirmation, merchant finalization, and final downloads.
