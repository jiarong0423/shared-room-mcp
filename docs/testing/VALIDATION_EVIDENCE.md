...
| Current local guided OCR+LLM proposal apply smoke | PASS | `/private/tmp/webmcp-codex-ocr-llm-local-accept-final/codex-ocr-llm-demo-2026-09-03T18-44-39-400Z.json` | Tesseract OCR produced 7 draft rows from the generated menu image; guided Codex visual review prepared 18 structured rows; the test-only accept flag applied 18 rows and kept member release closed |
| Current local guided OCR+LLM pending demo room | PASS | `/private/tmp/webmcp-codex-ocr-llm-local-pending-final-2/codex-ocr-llm-demo-2026-09-03T18-44-40-257Z.json` | same generated evidence and OCR path, but left the 18-row Codex visual-review draft pending for the host to approve |
| Current local contract stress after local-review bridge gate | 20/20 passed | `/private/tmp/webmcp-codex-ocr-llm-contracts-final/local-contract-stress-2026-09-03T18-47-01-301Z.md` | 20 Chinese and English scenarios, one fast round each, no provider keys; OCR-only parser output remains review-required |
| Current HITL Member-Visibility Release after visual-review proof gate | 4/4 passed | `/private/tmp/webmcp-codex-ocr-llm-member-release-final/member-release-stress-2026-09-03T18-46-59-817Z.md` | visual-review-backed drafts remain behind host review, then host release, member confirmation, owner finalization, HTML export, and PDF export pass |
...
- OCR-only output is draft evidence. A cloud `semantic_repair_draft` requires local OCR plus local/visual LLM structured items before host review can clear the OCR-only blocker.
