...
- Current Zeabur post-deploy smoke on 2026-09-04: latest deployment reached `RUNNING`; live `/healthz` returned `providerOrder=["local_vision","gemini","openai"]`, `localVisionConfigured=false`, `allowRemoteVisionFallback=false`, `/data/rooms.json`, and no provider key flags. Low-rate live Member-Visibility Release smoke passed 4/4 against generated test rooms. Authorized local bridge wrote one local OCR plus mock local-vision `semantic_repair_draft` to a generated live test room, and the proposal stayed pending host confirmation.
