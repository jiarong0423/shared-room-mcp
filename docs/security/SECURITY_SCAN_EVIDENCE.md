...
- HTML cache-control regression smoke: the home page route serves `index.html` with `Cache-Control: no-store` so judges and the in-app browser do not stay on a stale UI after deployment. The route-level fix passed a 400/400 local contract stress matrix.
